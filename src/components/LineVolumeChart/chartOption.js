...
const dealTooltip = (params, name, unit) => {
  ...
  let tip = `<div class="line-bar-tooltip">
      ...
      <div class="flex ${BinanceRate?'show':'hide'}"">
        ...
        <div class="value">${ unit === 'USD' ? `$${formatNumber(BinanceRate, 2)}` : `${formatNumber(BinanceRate, 2)} ${name}` }</div>
      </div>
      <div class="flex ${OKXRate?'show':'hide'}"">
        <b class="logo okx"></b>
        <div class="text-content">OKX</div>
        <div class="value">${ unit === 'USD' ? `$${formatNumber(OKXRate, 2)}` : `${formatNumber(OKXRate, 2)} ${name}` }</div>
      </div>
      <div class="flex ${BitgetRate?'show':'hide'}"">
        <b class="logo bit"></b>
        <div class="text-content">Bitget</div>
        <div class="value">${ unit === 'USD' ? `$${formatNumber(BitgetRate, 2)}` : `${formatNumber(BitgetRate, 2)} ${name}` }</div>
      </div>
      <div class="flex ${(BitgetRate || BinanceRate || OKXRate)?'show':'hide'}"">
        <div class="text-content">总计</div>
        <div class="value">${ unit === 'USD' ? `$${formatNumber(sum, 2)}` : `${formatNumber(sum, 2)} ${name}` }</div>
      </div>
    </div>`
    return tip
}
